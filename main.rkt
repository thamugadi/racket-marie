...
(define (write-mem m n x)
  (append (take m 3) (list (append (take (memory m) n) (list x) (cdr (drop (memory m) n))))))
...
(define (init-machine memsize) (list 0 0 0 (map (lambda (x) (* x 0)) (range memsize))))


(define-instruction load     #x1000) (define-instruction store    #x2000)
(define-instruction add      #x3000) (define-instruction sub      #x4000)
(define-instruction input    #x5000) (define-instruction output   #x6000)
(define-instruction halt     #x7000) (define-instruction skipcond #x8000)
(define-instruction jump     #x9000)
